...
**[Пошук компонент зв’язності](#Пошук-компонент-звязності)**<br>
**[Пошук компонент сильної зв’язності](#Пошук-компонент-сильної-звязності)**<br>
...
## Вступ
Наша команда працювала над темою “Аналіз зв’язності графів”, яка має велику кількість різноманітних застосувань у реальному житті. У рамках міні проєкту, ми розробили модуль-бібліотеку написану мовою Python для аналізу зв’язності графів.
<br><br>
Знань про графи, отриманих на курсі дискретної математики, не вистачило б для ефективної реалізації алгоритмів, зате їх вистачило для того, щоб упевнено почуватися в темі та розуміти описи складних, але ефективних алгоритмів. Найважливішим знанням, яке використувалося майже в усіх алгоритмах, є розуміння пошуку вглиб (depth-first search)
<br><br>
На початку роботи, ми розділили завдання наступним чином:
+ **Валерія Федорчак**: зчитування і запис графу у файл
+ **Юрій Зінчук**: пошук компонент зв’язності
+ **Марко Рузак**: пошук компонент сильної зв’язності в орієнтованому графі
+ **Роман Мутель**: пошук точок сполучення
+ **Михайло Гуменюк**: пошук мостів
...
## Пошук мостів
Описана у файлі [func_find_brides.py](https://github.com/gingrwho/discreteProject_17/blob/main/func_find_bridges.py)
```python
def find_bridges(graph: list) -> list
```
Знаходить всі мости в неорієнтованому графі <br><br>
Для розв'язання цього завдання було розроблено дві допоміжні функції .
+ Перша перетворює список ребер графа в словник, який репрезентує матрицю суміжності. Тобто ключем є кожна вершина, а значенням – множина суміжних їй вершин.
```python
def create_adj_matrix(graph: list) -> dict
```
+ Друга функція є рекурсивною і знаходить мости у графі. Вона є DFS алгоритмом. Він працює з часовою складністю `O(n + m)`,  де n - кількість вершин, m - кількість ребер.
```python
...
### Деякі означення:
+ **Корінь** - вершина, з якої був запущений dfs. На рисунку коренем є вершина “1”
+ **Пряме ребро** - ребро, за яким був перехід під час dfs. На рисунку такі ребра зроблені товстішими. 
+ **Зворотнє ребро** - ребра, за якими не було переходу. Вони завжди ведуть “нагору”, ближче до кореневої вершини. Наприклад, ребро (6,1).
+ **disc(discovery time)** (глибина вершини) - порядок вершини під час dfs обходу.
+ **low(low value)** (мінімальна глибина вершини) – мінімальна довжина шляху від вершини до кореня (з використанням як прямих, так і зворотніх ребер). Наприклад, для вершини 10 порядок входження в dfs = 8, але мінімальна глибина = 2 (довжина шляху (10,6)(6,1))
<br><br>
Можна сформулювати **критерій**: ребро( v, u) є *мостом*, якщо з будь-якої  дочірньої вершини  v неможливо дійти до батьківської вершини для v. Цей факт можна перевірити так: disc(v) < low(u)(мінімальна глибина дочірньої вершини більша ніж глибина батьківської).

**Приклад роботи:**

![Приклад алгоритму](img/graph2.jpeg)
![Приклад алгоритму](img/result2.png) 

## Висновки

Спільними зусиллями  команди ми розробили модуль-бібліотеку написану мовою Python для аналізу зв’язності графів та протестували її  на даних великого розміру. Цей проект допоміг нам краще зрозуміти тему графів з дискретної математики та навчитись поєднувати її з програмуванням. 
